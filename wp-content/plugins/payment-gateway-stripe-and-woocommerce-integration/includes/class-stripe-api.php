...
class EH_Stripe_Payment extends WC_Payment_Gateway {
    ...
    public function get_charge_details( $wc_order, $token, $order_type, $client, $card_brand, $currency, $amount,$customer = '' ) {
        ...
        if($show_items_details){
            ...
            $charge['description']=$charge['metadata']['Products'] .' '.get_bloginfo('blogname') . ' Order #' . $wc_order->get_order_number();
        }
        $charge['confirm'] =  true ;
        ...
        try {
            ...
            if ($process_auth) {
                ...
                try {
                    ...
                    $body = $e->getJsonBody();
                    $err  = $body['error'];
                    $this->process_error( $wc_order, $err, 'failed' );
                    return;
                }
