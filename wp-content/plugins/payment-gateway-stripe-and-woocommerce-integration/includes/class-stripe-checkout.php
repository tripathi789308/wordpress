...
class Eh_Stripe_Checkout extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
        ...
        'customer_email' => (WC()->version < '2.7.0') ? $order->billing_email : $order->get_billing_email() ,
        ...
        'success_url'=> home_url().'/?wc-ajax=eh_spg_stripe_checkout_order'.'&session_id={CHECKOUT_SESSION_ID}'.'&order_id='.$order_id.'&_wpnonce='.wp_create_nonce('eh_checkout_nonce').'&createaccount='.$createaccount,
        'cancel_url' => home_url().'/?wc-ajax=eh_spg_stripe_cancel_order'.'&_wpnonce='.wp_create_nonce('eh_checkout_nonce').'&createaccount='.$createaccount.'&order_id='.$order_id,
        ]);
